...
import 'package:vector_math/vector_math_64.dart'
    show Quad, Vector3, Matrix3, Matrix4;
...
/// {@end-tool}
@immutable
class RotateInteractiveViewer extends StatefulWidget {
  /// Create an InteractiveViewer.
  ///
  /// The [child] parameter must not be null.
  RotateInteractiveViewer({
    super.key,
  ...
  @override
  _RotateInteractiveViewerState createState() =>
      _RotateInteractiveViewerState();
}

class _RotateInteractiveViewerState extends State<RotateInteractiveViewer>
  ...
  Matrix4 _validateMatrix(Matrix4 matrix, Matrix4 defaultMatrix) {
    ...
    if (invalidSides.length > 1) {
      correction = invalidSides.fold(Offset.zero,
          (Offset value, LineSegment invalidSide) {
        final ClosestPoints closestPoints =
            invalidSide.findClosestPointsRect(_boundaryRect);
        final Offset difference = closestPoints.a - closestPoints.b;
        if (magnitudesAligned) {
          final bool xMisaligned = difference.dx != 0.0 &&
              value.dx != 0.0 &&
              (difference.dx > 0.0 != value.dx > 0.0);
          final bool yMisaligned = difference.dy != 0.0 &&
              value.dy != 0.0 &&
    ...
    if (!magnitudesAligned || invalidSides.length == 1) {
      ...
      final LineSegment invalidSide = (invalidSides
            ..sort((LineSegment a, LineSegment b) {
              final ClosestPoints closestPointsA =
                  a.findClosestPointsRect(_boundaryRect);
              final double distanceA = _distanceBetweenPoints(
                closestPointsA.a,
                closestPointsA.b,
              ).abs();
              final ClosestPoints closestPointsB =
                  b.findClosestPointsRect(_boundaryRect);
    ...
    final Quad correctedNextViewport =
  ...
  void _onScaleUpdate(ScaleUpdateDetails details) {
    ...
    switch (_gestureType!) {
      case _GestureType.scale:
        ...
        if (_roundOffset(_referenceFocalPoint!) !=
            _roundOffset(focalPointSceneCheck)) {
        ...
        _transformationController!.value = _matrixRotate(
          _transformationController!.value,
          _getMatrixRotation(_transformationController!.value) -
              desiredRotation,
          details.localFocalPoint,
        );
        ...
        // the scale should be on the same place in the scene before and after
        // the scale.
        final Offset focalPointSceneRotated =
  ...
  @override
  void didUpdateWidget(RotateInteractiveViewer oldWidget) {
...
/// See also:
///
///  * [RotateInteractiveViewer.transformationController] for detailed documentation
///    on how to use TransformationController with [RotateInteractiveViewer].
...
class LineSegment {
  ...
  static bool _rectContainsInclusive(Rect rect, Offset offset) {
    ...
    return rOffset.dx >= rRect.left &&
        rOffset.dx <= rRect.right &&
        rOffset.dy >= rRect.top &&
        rOffset.dy <= rRect.bottom;
  }

  /// A special case of the linesIntersectAt method.
  static Offset _isVerticalAndInterceptsNonVerticalLineAt(
      LineSegment vertical, LineSegment nonVertical) {
    ...
    final double intersectionY =
        nonVertical.slope * x + nonVertical.lineYIntercept;
    assert(x.isFinite);
  ...
  static bool _isVerticalAndInterceptsNonVertical(
      LineSegment vertical, LineSegment nonVertical) {
    final Offset intersection =
        _isVerticalAndInterceptsNonVerticalLineAt(vertical, nonVertical);
    return vertical.contains(intersection) &&
        nonVertical.contains(intersection);
  }
  ...
  bool _lineContains(Offset offset) {
    ...
    return (offset.dy - (slope * offset.dx + lineYIntercept)).abs() <=
        1 / _kTolerance;
  }
  ...
  Offset? linesIntersectAt(LineSegment lineSegment) {
    ...
    if (lineSegment.slope == slope ||
        (lineSegment.slope.isInfinite && slope.isInfinite)) {
      return null;
    }
  ...
  ClosestPoints findClosestPointsLineSegment(LineSegment lineSegment) {
    ...
    final Offset closestToIntersectionLineSegment = lineSegment
        .findClosestOffsetOnLineSegmentToOffsetOnLine(lineIntersection);
    ...
    return (<ClosestPoints>[toTop, toRight, toBottom, toLeft]
          ..sort((ClosestPoints first, ClosestPoints second) {
            final double firstDistance =
                _distanceBetweenPoints(first.a, first.b);
            final double secondDistance =
                _distanceBetweenPoints(second.a, second.b);
  ...
  Offset findClosestToOffset(Offset offset) {
    ...
    final double fraction =
        (dotProduct / lengthSquared).clamp(0.0, 1.0).toDouble();
